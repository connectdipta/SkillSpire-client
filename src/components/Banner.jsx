...
const Banner = () => {
  ...
  // ✅ FIX 1: Use useMemo to prevent particles from resetting on every render
  ...
  return (
    <section className="relative w-full overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-slate-900 via-[#1e1b4b] to-slate-900 shadow-2xl my-8 border border-white/10">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        ...
        {/* ✅ FIX 2: Particles Animation Logic */}
        {particles.map((p) => (
          <motion.div
            key={p.id}
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 py-28 text-center">
        ...
        <motion.form
          ...
          className="mt-10 flex flex-col sm:flex-row items-center gap-2 max-w-2xl mx-auto p-2 rounded-full bg-white/10 border border-white/20 backdrop-blur-xl shadow-[0_0_40px_rgba(0,0,0,0.3)]"
        >
          <div className="flex-1 flex items-center px-4">
            <FiSearch className="text-2xl text-white/70" />
            <input
              type="text"
              placeholder="Search contests (e.g. Coding, Quiz)..."
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              className="w-full bg-transparent border-none text-white text-lg px-4 py-3 focus:outline-none focus:ring-0 placeholder:text-white/50"
            />
          </div>
          <button
            type="submit"
            className="w-full sm:w-auto px-8 py-3.5 rounded-full bg-gradient-to-r from-yellow-400 to-orange-500 text-slate-900 font-bold text-lg shadow-lg hover:shadow-orange-500/50 hover:scale-105 transition-all duration-300"
          >
            Search
          </button>
        </motion.form>
